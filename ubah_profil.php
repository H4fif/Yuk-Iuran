<?php  # Script ubah_profil.php
/*
- Script untuk mengubah data profil.
- I.S. : data awal profil sebelum diubah.
- F.S. : data baru profil.
...
echo '<div class="page-header"><h3><span class="fas fa-user-alt"></span> <span class="glyphicon glyphicon-edit"></span> Ubah Profil</h3></div>';

$id = $_SESSION['kode_akun'];

require(MYSQLI);
$id = mysqli_real_escape_string($dbc, $id);
// Validasi kode customer dengan database :
$q = "SELECT * FROM tbl_akun AS a LEFT JOIN tbl_penyewa_kontrakan AS b USING (kode_akun) LEFT JOIN tbl_wilayah AS c USING (kode_wilayah) WHERE kode_akun = $id";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($_SESSION['hak_akses'] != 1){
        ...
        if (empty($val['no_ktp'])){
            ...
            $q = "SELECT no_ktp FROM tbl_penyewa_kontrakan WHERE no_ktp = '$no_ktp' AND kode_akun != $id";
            // Validasi query:
            if ($r = @mysqli_query($dbc, $q)) {
        ...
        if (empty($val['no_hp'])) {
            ...
            $q = "SELECT kode_pk FROM tbl_penyewa_kontrakan WHERE no_hp = '$nohp' AND kode_akun != $id";
            if ($r = @mysqli_query($dbc, $q)) {
                if (mysqli_num_rows($r) > 0) {
    ...
    if (!empty($val['email'])) {
        if (!filter_var($val['email'], FILTER_VALIDATE_EMAIL)) {
            ...
            $q = "SELECT email FROM tbl_akun WHERE LOWER(email) = LOWER('$email') AND kode_akun != $id";
            // Validasi query:
            if ($r = @mysqli_query($dbc, $q)) {
                // Validasi hasil query:
                if (mysqli_num_rows($r) > 0) {
                    ...
                    $q = "SELECT email FROM tbl_akun WHERE kode_akun = $id";
                    $r = mysqli_query($dbc, $q);
                    $old_email = mysqli_fetch_array($r, MYSQLI_ASSOC);
                }
            } else {
                showSqlError($dbc);
            }  
    ...
    if (!empty($val['kata_sandi_lama']) || !empty($val['kata_sandi_baru'])) {
        if (strlen($val['kata_sandi_baru']) < 4) {
            ...
            $q = "SELECT kata_sandi FROM tbl_akun WHERE kode_akun = $id AND kata_sandi = SHA1('$kata_sandi_lama')";
    ...
    if (empty($errors)) {
        if ($_SESSION['hak_akses'] != 1){
            ...
            $q = "UPDATE tbl_akun AS a INNER JOIN tbl_penyewa_kontrakan AS b USING (kode_akun) SET $qfoto b.nama = LOWER('$nama'), b.alamat = LOWER('$alamat'), b.kode_wilayah = $kode_wilayah, tanggal_lahir = '$tgl_lhr', jenis_kelamin = UPPER('$jk'), pekerjaan = LOWER('$pekerjaan'), id_kamar = '$id_kamar'";
            ...
            $q = "UPDATE tbl_akun SET email = '$email'";
        }
        ...
        echo '<a class="btn btn-default" href="profil.php"><span class="glyphicon glyphicon-circle-arrow-left"></span> Kembali</a>';
    } else {
        showErrorInput($errors);
        showUbahProfilForm($dbc, $data, $val);
    }
} else {
    showUbahProfilForm($dbc, $data, $val);
}
